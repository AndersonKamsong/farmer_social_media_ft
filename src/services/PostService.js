...
class PostService {
    // Create a new post (only accessible by farmers)
    // Create a new post
    ...
    // // Get all posts (assuming you want to show only the farmer's posts here)
    // async getAllPosts() {
    //     const response = await fetch(`${API_URL}`, {
    //         method: 'GET',
    //     });
    ...
    //         },
    //         body: JSON.stringify(postData), // postData should include { title, content, groupId, etc. }
    //     });
    ...
    //     const response2 = await fetch(`http://localhost:5000/api/uploadFile/${data.userId}`, {
    //         method: 'Post',
    //         headers: {
    //             'access-control': '*',
    //         },
    ...
    //     return response.json(); // Return created post
    // }
    async createPost(postData) {
        const token = JSON.parse(localStorage.getItem('user')).token;
